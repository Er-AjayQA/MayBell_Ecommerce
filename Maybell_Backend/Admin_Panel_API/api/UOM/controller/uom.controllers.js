// Imports & Configs
const UomModel = require("../model/uom.model");
...
exports.create = async (req, res) => {
  try {
    ...
    const dataCount = await UomModel.countDocuments({
      deletedAt: null,
    });

    if (!order && dataCount > 0) {
      const lastData = await UomModel.findOne({ deletedAt: null })
        .sort({ order: -1 })
        .limit(1);
      lastOrderValue = lastData ? lastData.order + 1 : lastOrderValue + 1;
    }

    const alreadyExist = await UomModel.findOne({
    ...
    const createData = await UomModel.create(data);
...
exports.getAll = async (req, res) => {
  try {
    ...
    const totalRecords = await UomModel.countDocuments(filter);

    const getAllData = await UomModel.find(filter)
      .limit(limit)
      .skip(skip)
      .sort({
        _id: sort ? "asc" : "desc",
...
exports.getDetails = async (req, res) => {
  try {
    ...
    const getDetails = await UomModel.findOne({ ...filter, _id: id });
...
exports.update = async (req, res) => {
  try {
    ...
    const ifAlreadyExist = await UomModel.find({
      _id: { $ne: id },
      deletedAt: null,
      $or: [{ name }, { order }],
    });
    ...
    const updateData = await UomModel.updateOne(
      { _id: id },
      {
        $set: data,
      }
...
exports.updateStatus = async (req, res) => {
  try {
    ...
    const updateData = await UomModel.updateOne({ _id: id }, [
...
exports.delete = async (req, res) => {
  try {
    ...
    const deleteData = await UomModel.updateOne(
      {
        _id: id,
      },
...
exports.deleteMultiple = async (req, res) => {
  try {
    ...
    const deleteData = await UomModel.updateMany(
      {
        _id: {
          $in: ids,
        },
